...
def test_heartbeat_sent_before_prune(data_store: InMemoryDataStore, worker: Worker):
    ...
    with (
        ...
        patch.object(
            worker.worker_node_repository, "get_leader_node_id", return_value="node_id"
        ),
...
def test_non_leader_skips_context_refresh(
    ...
    with (
        ...
        patch.object(
            worker.worker_node_repository,
            "get_leader_node_id",
            return_value="not_this_node_id",
        ),
...
def test_leader_skips_context_not_due_for_refresh(
    ...
    def get_refresh_after_in_future(name):
    ...
    with (
        ...
        patch.object(
            worker.context_repository, "get_refresh_after", get_refresh_after_in_future
        ),
        patch("sovereign.v2.worker.config") as mock_config,
        patch("sovereign.v2.worker.time.sleep", side_effect=_sleep_side_effect),
    ):
...
def test_leader_skips_context_when_due(
    ...
    # noinspection PyUnusedLocal
    def get_refresh_after_in_past(name):
    ...
    with (
        ...
        patch.object(
            worker.context_repository, "get_refresh_after", get_refresh_after_in_past
        ),
        patch("sovereign.v2.worker.config") as mock_config,
        patch("sovereign.v2.worker.time.sleep", side_effect=_sleep_side_effect),
    ):
